# Gemini CLI Automation: Google VALID VRM Cleanup

This file contains the instructions and the Python automation script to batch-process Google VALID avatars for use with the Blender Lip Sync extension.

## 1. The Python Script (vrm_cleanup.py)
Save the code block below as `vrm_cleanup.py` in your main avatar folder.
...
def process_vrm(filepath):
    ...
    # Export
    out_path = filepath.replace(".vrm", "_CLEANED.vrm")
